...
async def test_process_image_keeps_original_and_makes_model_copy(engine, settings):
    """手机原图保留原始分辨率（细节留给分块），喂给模型的副本压到 1600px 防 413。"""
    ...
    with Image.open(stored) as result:
        assert max(result.size) > 1600, "原图不再被原地压小"
    model_copy = stored.parent / ".derived" / f"{stored.stem}.enh.jpg"
    assert model_copy.is_file(), "要另存一份模型用的增强副本"
    with Image.open(model_copy) as result:
