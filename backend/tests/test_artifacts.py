...
def test_render_scaffold_files_are_not_artifacts():
    """渲染脚手架（本地 GSAP、音轨、工程清单）不是能交付给用户的产出物。"""
    assert is_ignored(Path("assets/gsap.min.js")) is True
    assert is_ignored(Path("assets/narration.mp3")) is True
    assert is_ignored(Path("hyperframes.json")) is True
    assert is_ignored(Path("meta.json")) is True
    assert is_ignored(Path("composition-skeleton.html")) is True
    # 模型自己写的组合、成片、配音仍然算产出物
    assert is_ignored(Path("explain.html")) is False
    assert is_ignored(Path("讲解视频.mp4")) is False
    assert is_ignored(Path("讲解配音.wav")) is False
    # 用户自己放进 assets/ 的图片没有被误伤
    assert is_ignored(Path("assets/封面.png")) is False
