...
async def _run_turn(engine, settings, monkeypatch, *, task_type: str = "auto") -> _FakeClient:
    with Session(engine) as session:
...
async def test_turn_collects_english_result(engine, settings, monkeypatch):
    ...
    (ws / "english_result.json").write_text(json.dumps(ENGLISH_RESULT), encoding="utf-8")

    await _run_turn(engine, settings, monkeypatch, task_type="english_passage")
...
async def test_turn_collects_grading_result_with_result_type(engine, settings, monkeypatch):
    ...
    await _run_turn(engine, settings, monkeypatch, task_type="grading")
...
async def test_turn_invalid_english_result_ignored(engine, settings, monkeypatch):
    ws = workspace.workspace_dir(settings, "u1", "t1")
    ws.mkdir(parents=True, exist_ok=True)
    (ws / "english_result.json").write_text('{"answers": "oops"}', encoding="utf-8")
