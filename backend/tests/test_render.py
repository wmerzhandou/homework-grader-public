...
async def test_output_is_capped_to_720p_unless_high(workspace, settings, monkeypatch):
    """默认出片压到 720p；请求里明确写 quality=high 才保留 1080p。"""
    source = workspace.parent / "hires.mp4"
    subprocess.run(
        ["ffmpeg", "-y", "-v", "error", "-f", "lavfi",
         "-i", "testsrc=duration=1:size=1920x1080:rate=15", "-pix_fmt", "yuv420p", str(source)],
        check=True,
    )
    ...
    async def fake_exec(*cmd, **kwargs):
        if "--output" not in list(cmd):  # ffprobe 之类照常走真实进程
            return await real_exec(*cmd, **kwargs)
        out_index = list(cmd).index("--output") + 1
        shutil.copy2(source, Path(cmd[out_index]))
        return _Proc()

    monkeypatch.setattr(render.asyncio, "create_subprocess_exec", fake_exec)
    for quality, expected in (("draft", 720), ("high", 1080)):
        _composition(workspace)
        request = {"composition": "explain.html", "output": f"{quality}.mp4",
                   "quality": quality, "duration": 1}
        result = await render.render_composition(
            workspace=workspace, settings=settings, request=request, notices=[]
        )
        assert [p.name for p in result] == [f"{quality}.mp4"]
        probed = subprocess.run(
            ["ffprobe", "-v", "error", "-select_streams", "v:0", "-show_entries",
             "stream=height", "-of", "default=nw=1:nk=1", str(workspace / f"{quality}.mp4")],
            capture_output=True, text=True, check=True,
        )
        assert int(probed.stdout.strip()) == expected
