    ...
    *,
    ...
    try:
        async with httpx.AsyncClient(base_url=base, timeout=timeout) as http:
            ...
            await notify_progress(notify, f"视频生成已提交（{seconds:.0f} 秒 {width}x{height}），预计 1-5 分钟…")
            ...
            while elapsed < settings.video_generation_timeout_s:
                ...
                if entry:
                    ...
                    break
                if int(elapsed) % 30 == 0:
                    await notify_progress(notify, f"视频生成中…已等待 {int(elapsed)} 秒")
            ...
            await notify_progress(notify, "视频生成完成，正在取回文件…")
            view = await http.get(
                "/view",
                params={
