...
MANIM_REQUEST_FILE = ".manim_request.json"
...
async def _assemble_narration(
    *,
    ...
    """分句音频 → 插静音 → 保音高变速 → 写成一条 wav。

    返回每段语音在**最终音轨**里的 (起点, 终点) 秒数（失败返回空列表）。
    变速是对整条流做的，所以位置统一除以 speed 换算回最终时间轴。
    """
    if not pieces:
        return []
    ...
    for index, data in enumerate(pieces):
        ...
        piece_duration = await _probe_duration(norm, settings) or 0.0
        spans.append((cursor, cursor + piece_duration))
    ...
    factor = speed if abs(speed - 1.0) > 0.01 else 1.0
    return [(start / factor, end / factor) for start, end in spans]
...
async def _synthesize_speech(
    *,
    ...
    """合成语音。timeline_out 非空时，填入逐句 {text,start,end}（给 Manim 那类
    "时间写死在脚本里"的渲染器对轴用）。"""
    ...
    if len(units) == 1 and abs(speed - 1.0) <= 0.01 and units[0][1] <= 0.01:
        ...
        if timeline_out is not None:
            duration = await _probe_duration(target, settings) or 0.0
            timeline_out.clear()
            timeline_out.append(
                {"text": units[0][0], "start": 0.0, "end": round(duration, 3)}
            )
        return [target]
    ...
    if pieces:
        ...
        if done:
            ...
            if timeline_out is not None:
                timeline_out.clear()
                timeline_out.extend(
                    {"text": unit_text, "start": round(start, 3), "end": round(end, 3)}
                    for (unit_text, _gap), (start, end) in zip(units, done)
                )
            return [target]
    ...
    if abs(speed - 1.0) <= 0.01:
        ...
        if timeline_out is not None:
            duration = await _probe_duration(target, settings) or 0.0
            timeline_out.clear()
            timeline_out.append({"text": text, "start": 0.0, "end": round(duration, 3)})
        return [target]
    work_dir = cache_dir / f"work-{uuid.uuid4().hex}"
    work_dir.mkdir(parents=True, exist_ok=True)
    try:
    ...
    *,
    ...
    for filename in (
        ...
        try:
            if filename == TTS_REQUEST_FILE:
            ...
            elif filename == RENDER_REQUEST_FILE:
                ...
                from . import manim_render  # 局部导入避免循环依赖
