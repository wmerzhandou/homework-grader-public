...
def _artifact_size(artifact: Artifact) -> int:
    ...
    total = 0
    paths = {artifact.stored_path}
    # 预览副本指向原文件时（表示"已检查、无需转码"）不要重复计入
    if artifact.preview_path and artifact.preview_path != artifact.stored_path:
        paths.add(artifact.preview_path)
    for raw in paths:
        if not raw:
            continue
        try:
