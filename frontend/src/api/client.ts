...
export const api = {
  ...
  /** 模型产出的文件（朗读音频、裁好的图片等） */
  artifactFileUrl(threadId: string, artifactId: string) {
    const ticket = getMediaTicket()
    const base = `/api/threads/${threadId}/artifacts/${artifactId}/file`
    return ticket ? `${base}?ticket=${encodeURIComponent(ticket)}` : base
  },

  /** 预览用的浏览器友好副本（图片缩放后 / 视频转 H.264 后）；没有副本时后端会回退到原文件 */
  artifactPreviewUrl(threadId: string, artifactId: string) {
    const ticket = getMediaTicket()
    const base = `/api/threads/${threadId}/artifacts/${artifactId}/file?preview=1`
    return ticket ? `${base}&ticket=${encodeURIComponent(ticket)}` : base
  },
  ...
  referencePreview(text: string) {
    return jsonRequest<{ references: ReferencePreviewEntry[]; unresolved: string[] }>(
...
function normalizeMessage(raw: unknown): ChatMessage {
  ...
  return {
    ...
    process_log: (r.process_log ?? null) as string | null,
    materials: r.materials as Material[] | undefined,
    artifacts: r.artifacts as ChatMessage['artifacts'],
    references: r.references as ChatMessage['references'],
