...
export default function Chat() {
  ...
  /** 模型当前在做什么（工具动作）+ 各动作次数，用于长任务进度提示 */
  const [activity, setActivity] = useState<{ text: string; counts: Record<string, number> } | null>(
    null,
  )
  /** 这一轮开始的时刻（算"已用时"） */
  ...
  /** 已用时（秒），没有在跑就是 0 */
  const elapsed = startedAt ? Math.max(0, Math.floor((nowTick - startedAt) / 1000)) : 0
    ...
    (event: ThreadEvent) => {
      switch (event.type) {
        ...
        case 'process_completed':
          ...
          break
        case 'activity':
          // 模型正在看图/写文件/跑命令：显示成"当前动作 + 次数"，长任务不再像卡死
          setQueued(false)
          setActivity({ text: event.text, counts: event.counts ?? {} })
          break
        ...
        <div className="px-3 pb-1 sm:px-4">
          ...
          {awaitingReply && !streamingId && !queued && (
            <p className="text-xs text-stone-400">
              ...
              {activity ? activity.text : '正在思考'}
              <span className="ml-1 text-stone-500">· 已用时 {formatElapsed(elapsed)}</span>
              {activity && <ActivityCounts counts={activity.counts} />}
            </p>
          )}
          {processing && (
...
/** 当前动作的累计次数（看图/命令/写文件），让人感觉"它在推进" */
function ActivityCounts({ counts }: { counts: Record<string, number> }) {
  ...
  return <span className="ml-2 text-stone-400">{parts.join(' · ')}</span>
}
