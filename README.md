...
> **关于这个公开版**：这是一个家庭自用项目的快照，`v20260915-0903`。
> 发布前已把部署相关的信息替换为通用值——局域网/公网地址用示例地址（`192.168.1.10` / `203.0.113.10`），
> ASR 默认指向本机，systemd 里的路径与运行账号改成 `/opt/homework-grader` 与 `homework`。
> 仓库内不含任何密钥：`data/`、`frontend/certs/`、`.env` 都在 `.gitignore` 里。
...
| `AUTH_TOKEN_TTL_S` | `2592000`（30 天） | 登录 token 有效期 |
...
- **凭据**：登录 token 有过期时间（默认 30 天），`POST /api/auth/logout` 服务端吊销；
